```
   ____      _               _                    _ _ 
  / ___|   _| |__   ___ _ __| |    __ _ _ __   __| (_)
 | |  | | | | '_ \ / _ \ '__| |   / _` | '_ \ / _` | |
 | |__| |_| | |_) |  __/ |  | |__| (_| | | | | (_| | |
  \____\__, |_.__/ \___|_|  |_____\__,_|_| |_|\__,_|_|
       |___/
```

# Hey, I'm Yohan 👋

## Blue Team Practitioner | Detection Engineering · SOC Operations

[![ISC2 CC](https://img.shields.io/badge/ISC2-Certified%20in%20Cybersecurity-00a651?style=for-the-badge&logo=isc2&logoColor=white)](https://www.credly.com/)
[![CASA](https://img.shields.io/badge/APIsec-Certified%20API%20Security%20Analyst-6A0DAD?style=for-the-badge&logoColor=white)](https://www.apisecuniversity.com/)
[![Security+](https://img.shields.io/badge/CompTIA-Security%2B%20(in%20progress)-e4002b?style=for-the-badge&logo=comptia&logoColor=white)](https://www.comptia.org/certifications/security)

---

I design, build, and validate detection systems — from architecture to alert.

Blue Team Practitioner focused on defensive security. I build hands-on lab environments to understand what security data actually looks like from a defender's perspective — not just how tools work, but how analysts think. My work covers SOC operations, detection engineering, log analysis, and network forensics.

---

## 🛠️ Tools & Technologies

![Suricata](https://img.shields.io/badge/Suricata-IDS%2FIPS-ef3e33?style=flat-square&logo=suricata&logoColor=white)
![Graylog](https://img.shields.io/badge/Graylog-SIEM-ff3633?style=flat-square&logo=graylog&logoColor=white)
![Wireshark](https://img.shields.io/badge/Wireshark-PCAP%20Analysis-1679A7?style=flat-square&logo=wireshark&logoColor=white)
![Snort](https://img.shields.io/badge/Snort-IDS-f5371b?style=flat-square)
![Kali Linux](https://img.shields.io/badge/Kali%20Linux-557C94?style=flat-square&logo=kalilinux&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Apache](https://img.shields.io/badge/Apache-D22128?style=flat-square&logo=apache&logoColor=white)
![rsyslog](https://img.shields.io/badge/rsyslog-Log%20Forwarding-333333?style=flat-square)
![MITRE ATT&CK](https://img.shields.io/badge/MITRE%20ATT%26CK-Mapped-red?style=flat-square)
![draw.io](https://img.shields.io/badge/draw.io-Architecture-F08705?style=flat-square&logo=diagramsdotnet&logoColor=white)

---

## 🔬 Lab Series

### Operation Iron Watch
A multi-phase SOC lab series — each episode builds on the last, closing gaps discovered in the previous operation.

| Episode | Focus | Status |
|---------|-------|--------|
| [Iron Watch 01](https://github.com/cyberlandji/operation-iron-watch-01) | Foundational SOC — Snort IDS, network visibility | ✅ Complete |
| [Iron Watch 02](https://github.com/cyberlandji/operation-iron-watch-02) | Graylog SIEM — web enumeration detection, auth.log gap discovered | ✅ Complete |
| [Iron Watch 03](https://github.com/cyberlandji/operation-iron-watch-03) | DMZ hardening, log pipeline, DDoS Detection Suite | ✅ Complete |

### Operation PCAP Autopsy
Network forensics series — structured PCAP investigations with detection rule writing.

| Episode | Focus | Status |
|---------|-------|--------|
| [PA-01](https://github.com/cyberlandji/operation-pcap-autopsy/tree/main/pa-01-you-dirty-rat) | STRRAT C2 — traffic analysis, IOC extraction, Suricata rules | ✅ Complete |
| [PA-02](https://github.com/cyberlandji/operation-pcap-autopsy/tree/main/pa-02-lumma-in-the-room-ah) | Lumma Stealer — infostealer traffic analysis | ✅ Complete |

---

## 🧠 What I Focus On

- **Detection Engineering** — writing and validating Suricata rules, Graylog event definitions, Sigma rules
- **Log Analysis & Alert Triage** — multi-source pipelines (EVE JSON, auth.log, Apache access.log)
- **Network Forensics** — PCAP analysis, traffic patterns, C2 identification
- **MITRE ATT&CK Mapping** — every detection tied to technique IDs
- **Architecture-First Labs** — draw.io diagrams before any configuration

---

## 🔗 Links

[![Portfolio](https://img.shields.io/badge/Portfolio-cyberlandji.com-000000?style=for-the-badge&logo=google-chrome&logoColor=white)](https://www.cyberlandji.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Yohan%20Cédric%20Landji-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/yohan-cedric-landji)
[![GitHub](https://img.shields.io/badge/GitHub-cyberlandji-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/cyberlandji)

---

*Building detection capability from scratch — one operation at a time.*
